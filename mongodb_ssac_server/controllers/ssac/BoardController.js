...
const boardController = {
  ...
  readAllBoard: async (req, res) => {
    try {
      const result = await board.find().populate("writer");
      if (!result)
  ...
  deleteBoard: async (req, res) => {
    ...
    // 일치하는 회원인지 아닌지 확인
    const ownResult = await board.checkAuth({boardId: id, writerId: userInfo._id});
  ...
  updateBoard: async (req, res) => {
    ...
    const ownResult = await board.checkAuth({boardId: id, writeId: userInfo._id});
    if (ownResult === -1){
      return res.status(409).json({message: "접근 권한이 없습니다"});
    } else if (ownResult === -2){
      return res.status(500).json({
